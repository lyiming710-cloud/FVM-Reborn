...
if (os_type == os_ios && instance_exists(obj_battle)) {
    ...
    // ESC: the only control that also exists outside battle.
    ...
    // Pause/continue: battle-only, directly above ESC.
    var _pause_active = global.is_paused && !global.show_menu;
    draw_set_alpha(global.show_menu ? 0.35 : (_pause_active ? 0.82 : 0.58));
    draw_set_color(c_black);
    ...
    draw_set_alpha((global.is_paused || global.show_menu) ? 0.35 :
        (obj_battle.speed_up ? 0.82 : 0.58));
    draw_set_color(c_black);
    ...
    // "缓时" only controls automatic 0.1x while a plant card is selected.
    draw_set_alpha((global.is_paused || global.show_menu) ? 0.35 :
        (obj_battle.card_slow_enabled ? 0.82 : 0.58));
    draw_set_color(c_black);
